      ...
      <div>
        ...
        <main className="flex ">
          <section className="flex-grow pt-14 px-6">
            <p className="text-xs">
              300+ Stays - {range} - for {noOfGuest} of guests
            </p>
          ...
          <section className="hidden xl:inline-flex xl:min-w-[500px]">
            <Map serachResults={serachResults} />
          </section>
        </main>
        <Footer />
      </div>
...
export async function getServerSideProps() {
  const serachResults = await fetch("https://www.jsonkeeper.com/b/5NPS").then(
    (res) => res.json()
  );
